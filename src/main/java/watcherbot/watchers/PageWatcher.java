...
public class PageWatcher implements Runnable {
    ...
    private void init(){
        ...
        manager.send(String.format("Watcher for %s has started", pageDescription.getDescription()));
    ...
    public void run() {
        ...
        if (newItems.size() == 0) {
            ...
            if (timeout >= timeoutCheck) {
                manager.send(String.format("There were no updates for %s for %d hours", pageDescription.getDescription(), timeoutCheck));
